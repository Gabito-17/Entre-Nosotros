...
export default function PlayerTableAndRow({ openModal }) {
  ...
  const currentRoundIndex = useGameBritneyStore((state) => state.currentRoundIndex);
  const roundScoresHistory = useGameBritneyStore((state) => state.roundScoresHistory);
  const totalScores = useGameBritneyStore((state) => state.totalScores);
  const getDisqualifiedPlayers = useGameBritneyStore((state) => state.getDisqualifiedPlayers);
  const getCurrentDealer = useGameBritneyStore((state) => state.getCurrentDealer);
...
return (
  <motion.div className="overflow-x-auto mt-4" initial="initial" animate="animate" variants={fadeUp}>
    <table className="table table-zebra w-full">
      <thead>
        <tr className="text-sm font-semibold">
          ...
          <th>
            <button className="flex items-center gap-2 w-full p-2" onClick={openTotalScoresModal}>
              <span>Total</span>
  ...
  {isDealer && (
    ...
    >
      ...
      />
              ...
              <td className="font-semibold text-center w-[80px]">{totalScores[player.name] || 0}</td>
            </motion.tr>
          );
        })}
      </motion.tbody>
